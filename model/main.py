...
def evaluate(model, eval_loader, topk_list, beam_size, device, code_to_item=None):
    ...
    Args:
        ...
        topk_list (list): List of k values for Recall@K and NDCG@K.
        beam_size (int): Number of beams for beam search.
        device: torch device.
        code_to_item (dict, optional): Mapping from SID tuple → itemID.
            When provided, tracks which PIDs can be recalled by the model.

    Returns:
        avg_recalls (dict): Average Recall@K.
        avg_ndcgs (dict): Average NDCG@K.
        recalled_pids (set): Set of itemIDs that appeared in any beam search output
            and mapped to a valid item. Empty set when code_to_item is None.
    """
    model.eval()
    ...
    with torch.no_grad():
        for batch in tqdm(eval_loader, desc="Evaluating"):
            ...
            # Track recalled PIDs: any beam output that decodes to a valid item
            if code_to_item is not None:
                preds_cpu = preds.detach().cpu()
                batch_size = preds_cpu.shape[0]
                for i in range(batch_size):
                    for j in range(beam_size):
                        code_tuple = tuple(preds_cpu[i, j].tolist())
                        if code_tuple in code_to_item:
                            recalled_pids.add(code_to_item[code_tuple])
    ...
    return avg_recalls, avg_ndcgs, recalled_pids
...
if __name__ == "__main__":
    ...
    for epoch in range(config['num_epochs']):
        ...
        # Evaluate the model
        avg_recalls, avg_ndcgs, _ = evaluate(model, validation_dataloader, config['topk_list'], config['beam_size'], device)
        ...
        if avg_ndcgs['NDCG@20'] > best_ndcg:
            ...
            test_avg_recalls, test_avg_ndcgs, _ = evaluate(model, test_dataloader, config['topk_list'], config['beam_size'], device)
